  ...
  import { CARTS_API, PAYMENT_URL } from '../../config/ApiConfig';
  ...
  const Cart = ({ purchase, setPurchase }) => {
    ...
    const handleCheckout = async () => {
      ...
      const payload = {
        amount: cartData.reduce((total, item) => total + item.price * item.quantity, 0) + gst,
        orderInfo: "Thanh toán mấy con gà...",
        redirectUrl: `${window.location.origin}/`, // URL redirect sau thanh toán thành công
        ipnUrl: `${window.location.origin}/api/payment-ipn`, // URL xử lý kết quả từ server
      };
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Xử lý thanh toán trả về
          handlePaymentResponse(data);
        } else {
          console.error("Lỗi khi gọi API thanh toán!");
        }
      } catch (error) {
    ...
    const handlePaymentResponse = (response) => {
      ...
      if (response.resultCode === 0) {
        ...
        console.error("Thanh toán thất bại:", response.message);
